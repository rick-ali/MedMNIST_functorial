...
class FunctorModel(pl.LightningModule):
    def __init__(self, model_flag, n_channels, n_classes, task, data_flag, size, run,
                 lr=0.001, gamma=0.1, milestones=None, output_root=None, latent_dim=512, lambda_t=0.5, lambda_W=0.1):
        super().__init__()
    ...
    def calculate_loss(self, batch, batch_idx, stage):
        ...
        labels_1 = y1[:,0].unsqueeze(1)
        
        outputs1, latent1 = self(x1)
        outputs2, latent2 = self(x2)
        
        
        natural_loss = self.get_natural_loss(outputs1, labels_1)
        if self.lambda_t > 0:
        ...
        loss = natural_loss + self.lambda_t * transformation_loss
